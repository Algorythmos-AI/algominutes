 ...
 *   - a storage_purges row records the audio to remove from Cloud Storage,
 *     which the caller then purges (see storage-purges-repo).
  ...
  const outcome = await withTx(
    async (client): Promise<DeleteNoteResult> => {
      ...
      const gone = await client.query<{ storage_path: string | null }>(
        'DELETE FROM notes WHERE id = $1 AND workspace_id = $2 RETURNING storage_path',
        [input.noteId, input.workspaceId],
      );
