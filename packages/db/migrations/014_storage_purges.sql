-- 014: a durable queue of what a deleted note leaves outside Postgres: its
-- Cloud Storage objects and its Firestore mirror doc. Expand-only (a new table).
...
-- removed elsewhere. The api purges it right away, and a row is deleted only
-- once the mirror doc and the objects are gone, so a failed or interrupted
-- purge is retried (by the PR-15 sweeper) rather than lost.
--
