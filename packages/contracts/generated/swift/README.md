# generated/swift — DO NOT HAND-EDIT

These Swift models are **generated**. They are produced by:

```bash
npm run models         # from packages/contracts — runs swift + kotlin
# or just this target:
npm run models:swift   # openapi-generator-cli -g swift5 → generated/swift
```
...
- The iOS app (`apps/ios`) consumes these models. A change here is a
  three-client change — the same generation feeds `generated/kotlin` for
  Android and the OpenAPI doc for web.

## Toolchain

`openapi-generator-cli` (the `@openapitools/openapi-generator-cli` npm wrapper)
with generator `swift5`. It requires a Java runtime on `PATH`. Generation is
