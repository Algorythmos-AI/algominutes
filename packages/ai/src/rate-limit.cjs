...
// Request rate limits for the HTTP services (api, billing). Shared so both
// behave the same way.
//
// Two layers:
//   - clientRateLimit: every request, keyed by the client IP. It covers the
//     public routes (share read, crash beacon, account delete, store webhooks)
...
//     runs right after the auth middleware, so the key can't be forged.
//
...
// count (trustProxyHops); with `true`, a client could send any X-Forwarded-For
// and get a fresh bucket each time.

const { rateLimit, ipKeyGenerator } = require('express-rate-limit');

const WINDOW_MS = 60_000;
const HEALTH_PATHS = new Set(['/health', '/healthz', '/health/ready']);
...
/** Per client IP, on every request except the health probes. */
function clientRateLimit({ limit = envLimit('RATE_LIMIT_IP_PER_MIN', 300) } = {}) {
  return rateLimit({
    windowMs: WINDOW_MS,
    limit,
