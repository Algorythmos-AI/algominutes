...
| **Deleted-note tombstone** | Postgres `deleted_notes` (note id, workspace id; no content) | Kept so a stale client can't upload into a deleted note, or re-queue it, after its purge finished | The sweeper prunes rows after 30 days |
