...
extension View {
    /// Applies AlgoMinutes's sheet chrome: charcoal surface, extra-large corner
    ...
    ///         ChatView(viewModel: chatModel).algoMinutesSheet()
    ///     }
    ///
    /// - Parameter detents: heights the sheet may rest at. Defaults to
    ///   `.large`, which is what most sheets here use.
    func algoMinutesSheet(_ detents: Set<PresentationDetent> = [.large]) -> some View {
        self
