...
final class TranscriptPageDecodingTests: XCTestCase {
    ...
    func testRealSpeakerNameIsKept() {
        let l = line(speaker: "Dr Slater", tag: nil, startMs: 1000).asTranscriptLine(index: 0)
        XCTAssertEqual(l.speaker, "Dr Slater")
    }
...
// a clinical recording that silently captured nothing is unrecoverable.
...
final class NoteMetaTests: XCTestCase {
    ...
    func testDurationGainsHoursOnlyWhenNeeded() {
        ...
        // A 2-hour clinical meeting reads better than "120:00".
...
final class TranscriptLineIdentityTests: XCTestCase {
    ...
    func testIdenticalUtterancesRemainDistinct() {
        // Six lines a clinical back-and-forth genuinely produces: the same
...
final class FormattingTests: XCTestCase {
    ...
    // 2-hour cap for Slater staff meetings; warning pill fires 5 min before.
...
final class SpeakerLabelTests: XCTestCase {
    ...
    func testKeepsARealName() {
        XCTAssertEqual(SpeakerLabel.display("Dr Slater"), "Dr Slater")
        let r = SpeakerLabel.split(speaker: "Dr Slater", text: "The patient reports no pain.")
        XCTAssertEqual(r.label, "Dr Slater")
        XCTAssertEqual(r.text, "The patient reports no pain.")
    }
...
final class UploadErrorCopyTests: XCTestCase {
    ...
    func testTimeoutMessageSaysTheRecordingIsSafe() {
        // This is the message a doctor sees after a failed consult upload. If
...
final class CapWarningTests: XCTestCase {
    func testReadsInMinutesRatherThanRawSeconds() {
        // Was "298s left — Wassup auto-stops at 120:00". A clinician mid-consult
...
final class RecordingDurabilityTests: XCTestCase {
    ...
    func testInsufficientStorageMessageNamesTheNumbers() {
        // A clinician needs to know how much is free and how much is needed,
...
// part a doctor actually reads. Each message must name the cause and say
...
final class AutoStopMessageTests: XCTestCase {
    ...
    func testEachReasonNamesItsOwnCause() {
        // A doctor whose recording stopped needs to know whether it was the
...
// offered to the doctor as "Upload it". AVAudioRecorder writes the moov atom at
...
// device needed, so the check that guards a patient consultation is actually
...
final class RecordingValidatorTests: XCTestCase {
    ...
    func testRejectsATruncatedRecording() async throws {
        ...
        // exactly what a force-quit mid-consult leaves behind.
        let url = try writeValidRecording(seconds: 3)
    ...
    func testRejectsARecordingTooShortToBeAConsultation() async throws {
...
// progress" — the doctor finished a consultation believing it was captured.
//
...
// consult into two notes when the call ends).
final class RecorderWatchdogTests: XCTestCase {
    ...
    func testWarnsBeforeGivingUpAndOnlyOnce() {
        ...
        XCTAssertFalse(decide(diverged: RecorderWatchdog.warnAfterSeconds + 60, warned: true).warnUser,
                       "the doctor should be told once, not every tick")
    }

    func testKeepsTryingWellPastTheWarning() {
        // The load-bearing trade-off: a 4-minute call must not end the consult,
...
// ten minutes, forever. The consultation never left the device and nothing
...
final class UploadStallPolicyTests: XCTestCase {
    ...
    func testABriefPauseIsToleratedForAHandover() {
        // Switching from clinic wifi to cellular goes quiet and recovers.
