...
final class RecorderService: NSObject, AVAudioRecorderDelegate {
    ...
    /// the upload permanently, so the consultation became unrecoverable.
    ///
    ...
    struct AutoStop: Equatable, Identifiable {
        ...
        /// What the doctor is told. Each case names what happened, because
    ...
    // when we last tried to fix it, and whether the doctor has been told.
    ...
    func start() async throws {
        ...
        // costs them nothing; discovering it mid-consult costs the consult.
    ...
    /// — the phone filling up mid-consult being the realistic case — is still
    /// decodable up to the failure point. Discarding it threw away everything
    /// recorded before the problem; a clinician 55 minutes into a consult lost
    /// all 55 minutes.
    ///
    ...
    private func autoStop(reason: AutoStop.Reason) {
        ...
        // the doctor learns their consultation ended when they next open the app.
    ...
    /// right here. Without it a force-quit mid-consult leaves an `.m4a` with no
    /// sample tables — unplayable, unrepairable, and previously offered back to
    /// the doctor as "Upload it".
    ///
    ...
    @objc private nonisolated func handleWillEnterForeground(_ notification: Notification) {
        ...
        Task { @MainActor in
            ...
            // waiting for the next tick — this is the moment the doctor is
    ...
    nonisolated func audioRecorderEncodeErrorDidOccur(_ recorder: AVAudioRecorder, error: Error?) {
        ...
        Task { @MainActor in
            ...
            // The realistic cause is the device filling up mid-consult, which
            // gives no other signal while backgrounded.
            RecordingNotifier.recordingStopped(
                message: "Recording stopped unexpectedly. Open Wassup Doc to save what was recorded."
            )
        }
    }
