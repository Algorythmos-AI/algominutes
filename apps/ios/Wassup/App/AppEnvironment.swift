...
final class AppEnvironment {
    ...
    private let pathMonitorQueue = DispatchQueue(label: "wassup.network-watch")
    ...
    /// A doctor whose upload failed on dead clinic wifi, and who stayed in the
    /// app while it recovered, sat there with a consultation on disk and
    ...
    /// silently captures nothing, and losing a clinical recording is
    ...
    /// which is right — but it used to do so unconditionally, so a clinician
    /// whose upload had failed lost the consult permanently with no warning.
    ...
    func recoverRecording(_ pending: RecordingStore.PendingRecording, onNoteCreated: @MainActor (String) -> Void = { _ in }) async {
        ...
        // uploading it would look like success while losing the consultation.
        //
        // Never delete on this path. The file may be the only record of a
        // patient visit, and a few tens of megabytes is a cheap price for
        ...
        if let noteId = pending.noteId {
            ...
            await uploadAndProcess(
                ...
                // legitimately-transcribing 90-minute consult to error at eight
